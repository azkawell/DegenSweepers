...
export const sendAllTransaction = async (transactions, wallet, conn, signers) => {
    try {
        ...
        toast.success('Approving transaction...', {duration: 4000});
...
export const sendAllTransactionForStake = async (transactions, wallet, conn, signers) => {
    try {
        console.log("signers", signers);
        console.log(transactions);
        let commitment = "max"
        ...
        for(let i = 0; i < transactions.length; i++) {
            ...
            if( transactions.length -1 === i) {
                if((signers.length - i*3) === 1) {
                    transaction.setSigners(wallet.publicKey, signers[i*3].publicKey);
                    if(signers.length !== 0) await transaction.partialSign(signers[i*3]);    
                } else if ((signers.length - i*3) === 2) {
                    transaction.setSigners(wallet.publicKey, signers[i*3].publicKey, signers[i*3 + 1].publicKey);
                    if(signers.length !== 0) await transaction.partialSign(signers[i*3], signers[i*3 + 1]);
                } else {
                    transaction.setSigners(wallet.publicKey, signers[i*3].publicKey, signers[i*3 + 1].publicKey, signers[i*3 + 2].publicKey);
                    if(signers.length !== 0) await transaction.partialSign(signers[i*3], signers[i*3 +1 ], signers[i*3 + 2]);
                }
            } else {
                transaction.setSigners(wallet.publicKey, signers[i*3].publicKey, signers[i*3 + 1].publicKey, signers[i*3 + 2].publicKey);
                if(signers.length !== 0) await transaction.partialSign(signers[i*3], signers[i*3 +1 ], signers[i*3 + 2]);
        ...
        const signedTxns = await wallet.signAllTransactions(unsignedTxns)
        toast.success('Approving transaction...');
...
const getStakedNftsForOwner = async (wallet) => {
    ...
    try {
        ...
        console.log(e);
        console.log("yvyxv")
        toast.error("RPC failed! Please reload the site.")
    }
    return allTokens;
}
...
const getClaimAmount = async (wallet) => {
    ...
    for(let stakeAccount of resp){
        ...
        // if (stakedNft.unstaked) continue;
        if (!stakedNft.unstaked) {
            let num = (moment().unix() - stakedNft.stakeTime.toNumber());
            num = num - stakedNft.withdrawnNumber
            let count = 0;
            while (num > 2592000) {
                claimAmount += (poolData.rewardAmount + count) * 30
                count++;
                num -= 2592000;
            }
            claimAmount += (poolData.rewardAmount + count) * num  / poolData.period;
        } else if(!stakedNft.unstakedClaimed) {
            let num = (stakedNft.unstakedTime.toNumber() - stakedNft.stakeTime.toNumber());
            let count = 0;
            while (num > 2592000) {
                claimAmount += (poolData.rewardAmount + count) * 30
                count++;
...
const stake = async (nfts , wallet) => {
    ...
    for(let nft of nfts) {
        ...
        console.log(stakeData.publicKey.toBase58());
    ...
    for (let i = 0; i < transaction.length; i++) {
        ...
        if (i % 3 === 2 || i === transaction.length - 1) {
            transactionSet.push(bigTx)
        }
    }
    await sendAllTransactionForStake(transactionSet, wallet, connection, signers)
}
...
async function claim(wallet) {
    ...
    if (result.result) {
        toast.success(`Successfully Claimed ${(amount / LAMPORTS_PER_SOL).toFixed(2)} $DGN`);
    }
}
...
const useNftStake = () => {
    ...
    const claimRewards = async (extraReward) => {
        if (!anchorWallet) {
            toast.error('Connect wallet first, please.');
            return;
        }
        ...
        await claim(anchorWallet, extraReward);
